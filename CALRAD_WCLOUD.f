...
!     DECLARE VARIABLES.
!
! Specify all the satellites to simulate
      integer, parameter:: ndat=2 
      character(10), parameter, dimension(1:ndat) :: allobs=(/'goes_img','goes_img'/)
      character(10), parameter, dimension(1:ndat) :: allisis=(/'imgr_g12','imgr_g11'/)
      real, parameter, dimension(1:ndat) :: sublat=(/0.,0./)
      real, parameter, dimension(1:ndat) :: sublon=(/-75.0,-135.0/)
...
!     
       ...
       if((isis=='imgr_g12' .and. (iget(327) > 0 .or. iget(328) > 0 .or. iget(329) > 0 .or. iget(330) > 0))  &
       .or. (isis=='imgr_g11' .and. (iget(446) > 0 .or. iget(447) > 0 .or. iget(448) > 0 .or. iget(449) > 0)) )then
!       isis='amsua_n15'
...
! compute satellite zenith angle
        ...
        ,sublat(isat),sublon(isat),sat_zenith) 
...
! Deallocate arrays
     ...
     &      err3
      ...
      end if  ! end of if block for calling crtm       
!     
  end do  ! end lopp for different satellites  
