...
!
      ...
      real rinc(5)
      integer iii,l,k,ist,ierr,Status,iostatusD3D,nrec
...
!jun      if(MODELNAME .EQ. 'GFS')then
!jun        read(5,111,end=119)fileNameD3D
!jun        print*,'D3D names in GFS= ',trim(fileNameD3D)
!jun      end if
...
! opening GFS flux file	 
	     iunit=33
             call baopenr(iunit,trim(fileNameFlux),status)
	     iunitd3d=34
...
! EXP. initialize netcdf here instead
	...
	STOP 9997
      ...
      ELSE IF(TRIM(IOFORM) == 'grib')THEN 
       IF(MODELNAME == 'GFS') THEN
        CALL INITPOST_GFS(NREC,iunit,iunitd3d,iostatusD3D,gfile)
       END IF	 
      ELSE
       PRINT*,'UNKNOWN WRF OUTPUT FORMAT, STOPPING'
       STOP 9999
