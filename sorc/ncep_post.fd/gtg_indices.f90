...
!          --- Apply smoothing
          ...
          write(*,*) 'computing trophtgrad/tropht'
          TI1 = SPVAL
...
!-----------------------------------------------------------------------
    write(*,*) 'enter tropfold: kmin,kmax=',kmin,kmax
...
!-----------------------------------------------------------------------
  subroutine Nsqcomp(kmin,kmax,LM,T,thetav,p,z,qv,qc,rhm,Nsqd,Nsqm)
...
!$$$
    ...
    integer, intent(in) :: kmin, kmax
    integer, intent(in) :: LM
    real, dimension(LM), intent(in) :: T,thetav,p,z,qv,qc,rhm
    ...
    real, dimension(LM) :: dthdz, thetam
...
!         --- boundaries of the data
          ...
          dqcdz = dirregzk(kmin1,kmax1,LM,k,qc,z)
          dqwdz = dqvdz+dqcdz
...
!     --- Quart. J. Roy. Meteor. Soc., 91, 507-513.
    ...
    write(*,*) 'enter ColPan: Ricrit=',Ricrit
...
!   --- Check input etai grid at this (i,j) point
    do k=1,nzi
       if(ABS(etai(k)-SPVAL)<SMALL1) then
          write(*,*) 'undefined etai: k,etai=',k,etai(k)
          return
       endif
    enddo
...
print *, "Final gtg output=",qitfax(IM/2,jsta,1:LM)
    deallocate(cat)
