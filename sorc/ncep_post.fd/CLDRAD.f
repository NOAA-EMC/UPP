...
!     LOGICAL,dimension(im,jm) ::  NEED
      ...
      INTEGER,dimension(isx:iex,jsta:jend) :: IBOTT, IBOTCu, IBOTDCu, IBOTSCu, IBOTGr,   &
                                         ITOPT, ITOPCu, ITOPDCu, ITOPSCu, ITOPGr
      REAL,dimension(im,jm)           :: GRID1
      REAL,dimension(isx:iex,jsta:jend)    :: GRID2, EGRID1, EGRID2, EGRID3,      &
...
!     B ZHOU: For aviation:
      REAL, dimension(isx:iex,jsta:jend) :: TCLD, CEILING
...
!
      REAL,dimension(isx:iex,jsta:jend)  :: P1D,T1D,Q1D,EGRID4
!     REAL, allocatable  :: RH3D(:,:,:)                     ! RELATIVE HUMIDITY
      ...
      REAL,dimension(isx:iex,jsta:jend) :: AOD_DU, AOD_SS, AOD_SU, AOD_OC, AOD_BC, AOD
      REAL,dimension(isx:iex,jsta:jend) :: SCA_DU, SCA_SS, SCA_SU, SCA_OC,SCA_BC, SCA2D
      REAL,dimension(isx:iex,jsta:jend) :: ASY_DU, ASY_SS, ASY_SU, ASY_OC, ASY_BC,ASY2D
      REAL,dimension(isx:iex,jsta:jend) :: ANGST, AOD_440, AOD_860      ! FORANGSTROM EXPONENT
      REAL               :: ANG1, ANG2
...
!       allocate (RH3D(im,jsta:jend,lm))
        allocate (rdrh(isx:iex,jsta:jend,lm))
        allocate (ihh(isx:iex,jsta:jend,lm))
...
!         ID(02)=141
!         if(grib=='grib1') then
!          CALL GRIBIT(IGET(661),LVLS(1,IGET(661)),GRIDisx,iex,JM)
...
!         ID(02)=141
!         if(grib=='grib1') then
!          CALL GRIBIT(IGET(662),LVLS(1,IGET(662)),GRIDisx,iex,JM)
!         elseif(grib=='grib2') then
!          cfld=cfld+1
...
!     Assume f is in x-log scale
      use ctlblk_mod, only: SPVAL,JSTA,JEND,IM,isx,iex
      implicit none
      ...
      use ctlblk_mod, only: jsta, jend, SPVAL, im, jm, grib,     &
                  cfld, datapd, fld_info, jsta_2l, jend_2u,isx,iex
      use rqstfld_mod, only: IGET, ID, LVLS, IAVBLFLD
      implicit none
!
