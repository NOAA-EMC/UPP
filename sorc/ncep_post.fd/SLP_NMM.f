...
!$omp parallel do 
      ...
      DO I=1,IM
        ...
        if(debugprint==.true. .and. i==ii .and. j==jj)print*,'Debug: FIS,IC for PSLP='      &
        ,FIS(i,j),PSLP(I,J)
        TTV(I,J)=0.
        LMHO(I,J)=0
	DONE(I,J)=.FALSE.
...
!
        ...
        if(debugprint==.true. .and. i==ii .and. j==jj)print*,'Debug: HTMO= ',HTMO(I,J,L)
      ENDDO
      ENDDO
!
  100 CONTINUE
        ...
        ELSE IF(FIS(I,J)<-1.0) THEN
          DO L=LM,1,-1
            IF(ZINT(I,J,L)>0.)THEN
              ...
              DONE(I,J)=.TRUE.
              if(debugprint==.true. .and. i==ii.and.j==jj)print*                           &
...
!HC OUTPUT PRESSURE LEVEL, USE SURFACE PRESSURE TO DO EXTRAPOLATION
 325  CONTINUE
      ...
      DO 330 J=JSTA,JEND
      DO 330 I=1,IM
      if(debugprint==.true. .and. i==ii.and.j==jj)print*,'Debug: with 330 loop'
      IF(DONE(I,J)) cycle   
      if(debugprint==.true. .and. i==ii.and.j==jj)print*,'Debug: still within 330 loop'
...
!     1,PINT(I,J,LLMH+1),ZINT(I,J,LLMH+1),TLYR,PSLP(I,J)
      ELSE
       TLYR=TPRES(I,J,LP)-0.5*FIPRES(I,J,LP)*SLOPE
       PSLP(I,J)=spl(lp)/EXP(-FIPRES(I,J,LP)/(RD*TLYR))                   
       if(debugprint==.true. .and. i==ii.and.j==jj)print*,'Debug:spl,FI,TLYR,PSLPA3='       &
         ,spl(lp),FIPRES(I,J,LP),TLYR,PSLP(I,J)
      END IF
      DONE(I,J)=.TRUE.
      KOUNT=KOUNT+1
