...
!
      ...
      use ctlblk_mod, only: num_procs, jend, iup, jsta, idn, mpi_comm_comp, im,&
          icoords,ibcoords,bufs,ibufs,me, &   ! GWV TMP

              jsta_2l, jend_2u,ileft,iright,ista_2l,iend_2u,ista,iend,jm
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      implicit none
!     
      include 'mpif.h'
...
!  send first col    to  ileft  last  col+  and receive last  col+ from ileft first col 
    ...
    &                  MPI_COMM_COMP,status,ierr)
     ...
     if(iright .gt. 0) then
        do j=jsta,jend
        a(iend+1,j)=colr(j)
!GWV            ibcoords(iend+1,j)=icolr(j)  !GWV TMP
            ibcoords(iend+1,j)=icolr(j)  !GWV TMP
!                write(0,*) ' GWVX IBCOLL SETT2 ',iend+1,j,icolr(j)
               ii=ibcoords(iend+1,j)/10000
                   jj=ibcoords( iend+1,j)-(ii*10000)
              if( j .ne. jj .or. ii .ne. iend+1 ) &
             write(0,921) j,iend+1,ii,jj,ibcoords(iend+1,j),' GWVX EXCH COORD FAIL j,iend+1,ii,jj,ibcoord '
 921   format(5i10,a50)
! 
...
!  send last col    to  iright first col-  and receive first col- from ileft last col 
    ...
    &                  MPI_COMM_COMP,status,ierr)
     ...
     if(ileft .ge. 0) then
        do j=jsta,jend
        a(ista-1,j)=coll(j)
!GWV            ibcoords(ista-1,j)=icoll(j)  !GWV TMP
...
! A(iend+1,jend+1) is in the iright    a(ista,jend+1) location 
    ...
    &                  MPI_COMM_COMP,status,ierr)
!GWV TEST       
!         write(0,139)' GWVX PRE CORNER ' ,ibcoords(iend,jsta-1),iend,jsta-1,ibcoords(iend,jsta+1),iend,jsta+1,&
!       ibcoords(ista,jend+1),ista,jend+1,ibcoords(ista,jend-1),ista,jend-1,me,ileft,iright
...
!    corner check for coordnates
              ...
              if(ii .ne. icc .or. jj .ne. jcc) write(0,151) ' CORNER FAIL ilb  ll ',icc,jcc,ibcoords(icc,jcc),ii,jj

              icc=ibu
              jcc=jbl
              ii=ibcoords(icc,jcc)/10000
              jj=ibcoords(icc,jcc)-(ii*10000)
              if(ii .ne. icc .or. jj .ne. jcc) write(0,151) ' CORNER FAIL ilb ul  ',icc,jcc,ibcoords(icc,jcc),ii,jj

              icc=ibu
              jcc=jbu
              ii=ibcoords(icc,jcc)/10000
              jj=ibcoords(icc,jcc)-(ii*10000)
              if(ii .ne. icc .or. jj .ne. jcc) write(0,151) ' CORNER FAIL ilb uu  ',icc,jcc,ibcoords(icc,jcc),ii,jj

              icc=ibl
              jcc=jbu
              ii=ibcoords(icc,jcc)/10000.
              jj=ibcoords(icc,jcc)-(ii*10000)
              if(ii .ne. icc .or. jj .ne. jcc) write(0,151) ' CORNER FAIL ilb lu  ',icc,jcc,ibcoords(icc,jcc),ii,jj

              
!      print *,'mype=',me,'in EXCH, after second mpi_sendrecv'
       if(ileft .ge. 0) then
!      write(0,119) ileft,me,ibcoords(ista-1,jend+1),ibcoords(ista-1,jend-1),ista-1,jend-1,jend+1
 119  format(' GWX LEFT EXCHANGE ileft,me,ibcoords(ista-1,jend+1),ibcoords(ista-1,jend-1),ista-1,jend-1,jend+1', &
      10i10)                                                                           
       endif
       if(iright .ge. 0) then
 !     write(0,129) iright,me,ibcoords(ista+1,jend+1),ibcoords(ista+1,jend-1),ista-1,jend-1,jend+1
...
!              do i=ista,iend
 ...
 151   format(a20,10i10)
              ...
              do i=ista,iend
               ii=ibcoords(i,j)/10000
                   jj=ibcoords( i,j)-(ii*10000)
           if(ii .ne. i .or. jj .ne. j) write(0,151) 'GWVX FAILED JBU IJ ',i,j,ibcoords(i,j),ibl,jbl,ibu,jbu
            end do
              j=jbl
              do i=ista,iend
               ii=ibcoords(i,j)/10000
                   jj=ibcoords( i,j)-(ii*10000)
           if(ii .ne. i .or. jj .ne. j) write(0,151) 'GWVX FAILED JBL IJ ',i,j,ibcoords(i,j),ibl,jbl,ibu,jbu
            end do
              i=ibl
              do j=jsta,jend
               ii=ibcoords(i,j)/10000
                   jj=ibcoords( i,j)-(ii*10000)
           if(ii .ne. i .or. jj .ne. j) write(0,151) 'GWVX FAILED IBL IJ ',i,j,ibcoords(i,j),ibl,jbl,ibu,jbu
            end do
              i=ibu
              do j=jsta,jend
               ii=ibcoords(i,j)/10000
                   jj=ibcoords( i,j)-(ii*10000)
           if(ii .ne. i .or. jj .ne. j) write(0,151) 'GWVX FAILED IBU IJ ',i,j,ibcoords(i,j),ibl,jbl,ibu,jbu
            end do
      if ( ierr /= 0 ) then
