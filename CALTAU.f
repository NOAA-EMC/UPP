...
!     DECLARE VARIABLES.
      ...
      real DZ,RDZ,RSFC,TV,RHO,ULMH,VLMH,DELUDZ,DELVDZ,ELSQR,ZINT1,    &
           ZINT2,Z0V,PSFC,TVV,QVV,ELV,ELV1,ELV2,ELSQR
!     
!********************************************************************
!     START CALTAU HERE.
!    
...
!
        ...
        IW=I+IVW(J)
        ZINT1=(ZINT(IW,J,LMHK)+ZINT(IE,J,LMHK)                  &  
         +ZINT(I,J+1,LMHK)+ZINT(IE,J+1,LMHK))*D25
        ZINT2=(ZINT(IW,J,LMHK+1)+ZINT(IE,J,LMHK+1)              &
         +ZINT(I,J+1,LMHK+1)+ZINT(IE,J+1,LMHK+1))*D25
        DZ  = D50*(ZINT1-ZINT2)       
        Z0V=(Z0(IW,J)+Z0(IE,J)+Z0(I,J+1)+Z0(IE,J+1))*D25
        DZ  = DZ-Z0V
        RDZ = 1./DZ
!
!        COMPUTE REPRESENTATIVE AIR DENSITY.
!
        PSFC = (PMID(IW,J,LMHK)+PMID(IE,J,LMHK)                &
         +PMID(I,J+1,LMHK)+PMID(IE,J+1,LMHK))*D25 
        TVV = (T(IW,J,LMHK)+T(IE,J,LMHK)                       &
         +T(I,J+1,LMHK)+T(IE,J+1,LMHK))*D25
        QVV = (Q(IW,J,LMHK)+Q(IE,J,LMHK)                       &
         +Q(I,J+1,LMHK)+Q(IE,J+1,LMHK))*D25
        TV   = (H1+D608*QVV)*TVV
        RHO  = PSFC/(RD*TV) 
...
!
	  ...
	  UZ0V=D25*(UZ0(IE,J)+UZ0(IW,J)+UZ0(I,J+1)+UZ0(IE,J+1))
	  VZ0V=D25*(VZ0(IE,J)+VZ0(IW,J)+VZ0(I,J+1)+VZ0(IE,J+1))
	ELSE   
          UZ0V=UZ0(I,J)
          VZ0V=VZ0(I,J)
	END IF  
        DELUDZ = (UH(I,J,LMHK)-UZ0V)*RDZ
        DELVDZ = (VH(I,J,LMHK)-VZ0V)*RDZ 

!       COMPUTE U (EGRIDU) AND V (EGRIDV) WIND STRESSES.
!                                       
        ELV1=(EL_MYJ(IW,J,LMHK)+EL_MYJ(IE,J,LMHK)              &
         +EL_MYJ(I,J+1,LMHK)+EL_MYJ(IE,J+1,LMHK))*D25
        ELV2=(EL_MYJ(IW,J,LMHK-1)+EL_MYJ(IE,J,LMHK-1)          &
         +EL_MYJ(I,J+1,LMHK-1)+EL_MYJ(IE,J+1,LMHK-1))*D25
        ELV=(ELV1+ELV2)/2.0  ! EL is defined at the bottom of layer
        ELSQR       =ELV*ELV
        TAUX(I,J)=RHO*ELSQR*DELUDZ*DELUDZ 
        TAUY(I,J)=RHO*ELSQR*DELVDZ*DELVDZ
