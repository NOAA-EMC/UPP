...
! Add Iredell's subroutine to read sigma files
!-------------------------------------------------------------------------------
subroutine rtsig(lusig,head,k1,k2,kgds,ijo,nct,&
                 h,p,px,py,t,u,v,d,sh,o3,ct,iret)
!$$$  Subprogram documentation block
!
...
!  Read and transform surface fields
  iret=1
  print*,'Debug rtsig= ',lusig,k1,k2,ijo,nct,kgds(1:20)
...
!$$$
    ...
    real dpmdps(km),dpddps(km),dpidps(km+1),vgradp,pd(km),px(km),py(km),os
    integer k,iret
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  do l=1,mpisize
    do k=1,kma
      do j=1,jm
        ...
        if(ja.le.jma) then
          do i=1,im
	    if(a(i,ja,k)<100.)print*,'bad T in mptranr4 input',i,ja,k,a(i,ja,k)
            ta(i,j,k,l)=a(i,ja,k)
          enddo
        endif
      enddo
    ...
    do k=1,km
      ...
      if(kb.le.kmb) then
        do j=1,jmb
          do i=1,im
            b(i,kb,j)=tb(i,j,k,l)
	    if(b(i,kb,j)<100.)print*,'bad T in mptranr4 output',i,kb,j,b(i,kb,j)
          enddo
        enddo
      endif
    enddo
