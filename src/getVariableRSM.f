subroutine getVariableRSM(fileName,DateStr,dh,VarName,VarBuff,IM,JSTA_2L,JEND_2U,LM,IM1,JS,JE,LM1)
...
!   04-02-10  BINBIN ZHOU - CHANGE FROM INDEX ORDER FROM I,K,J to I,J,K for RSM
!
...
!
   ...
   character (len=1)  :: dimnames
   real, allocatable, dimension(:,:,:,:) :: data
   integer :: ierr
   character(len=4)   :: Stagger

   write(*,*) 'Argument:fileName,DateStr,dh,VarName,IM,JSTA_2L,JEND_2U,LM,IM1,JS,JE,LM1 = ', &
       fileName,DateStr,dh,VarName,IM,JSTA_2L,JEND_2U,LM,IM1,JS,JE,LM1
...
! WRF IO API will output to "data"
   call ext_int_read_field(dh,DateStr,TRIM(VarName),data,WrfType,0,0,0,ordering,&
                             Stagger,' ' ,          & !BZhou: change from staggering to stagger
                             start_index,end_index, & !dom    and replace dimname with ' '
...
!CHUANG: accomendate different array arrangement with different ndim
   if(ndim.eq.1)then
    ...
    if (im1.gt.end_index(1)) write(*,*) 'Err:',Varname,' IM1=',im1,&
                ' but data dim=',end_index(1)
!bz    if (je.gt.end_index(3)) write(*,*) 'Err:',Varname,' JE=',je,&
!bz                ' but data dim=',end_index(3)
!bz    if (lm1.gt.end_index(2)) write(*,*) 'Err:',Varname,' LM1=',lm1,&
!bz                ' but data dim=',end_index(2)
    if (je.gt.end_index(2)) write(*,*) 'Err:',Varname,' JE=',je,&
                ' but data dim=',end_index(2)
...
!bz       VarBuff(i,j,l)=data(i,ll,j,1)  !RSM index is from upward(sfc is 1, top is LM1)
          VarBuff(i,j,l)=data(i,j,ll,1)  !so same as WRF, need to reverse to identical
                                         !as ETA post, in which top is 1, sfc is LM1
      enddo
     enddo
