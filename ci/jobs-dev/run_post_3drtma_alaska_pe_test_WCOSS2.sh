...
#PBS -o out.3drtma.pe_test_alaska
#PBS -e out.3drtma.pe_test_alaska
...
cat > itag <<EOF
&model_inputs
fileName='$homedir/data_in/3drtma/akrtma3d.t${HH}z.wrf_inout.nc'
IOFORM='netcdf'
grib='grib2'
...
if [ $err = "0" ] ; then
 ...
 cmp ${filein2} $homedir/data_out/3drtma/ak${filein2}_1.${machine}
 ...
 if [ $err1 -eq 0 ] ; then
  ...
  $cmp_grib2_grib2 $homedir/data_out/ak${filein2}.${machine} ${filein2} > ${filein2}.diff
  cmp ${filein2}.diff $homedir/data_out/ak${filein2}.diff
  export err2=$?
  if [ $err2 -eq 0 ] ; then
